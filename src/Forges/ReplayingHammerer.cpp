...
PatternAddressMapper &ReplayingHammerer::determine_most_effective_mapping(
    HammeringPattern &patt, bool optimize_hammering_num_reps,
    bool offline_mode) {
  Logger::log_debug("determine_most_effective_mapping called!");

  if (patt.address_mappings.empty()) {
    Logger::log_failure(format_string(
        "No mappings found in loaded pattern %s. Cannot continue.",
  ...
  int cur_reps = optimize_hammering_num_reps ? initial_hammering_num_reps
                                             : hammering_num_reps;
  ...
  for (auto it = patt.address_mappings.begin();
       it != patt.address_mappings.end(); ++it) {
    derive_FuzzingParameterSet_values(patt, *it);

    Logger::log_info(format_string("Mapping %s {agg ID -> (bank,rank,column)}:",
                                   (*it).get_instance_id().c_str()));
    Logger::log_data((*it).get_mapping_text_repr());

    size_t triggered_bitflips = 0;
    if (!offline_mode) {
      // try the original location where this pattern initially triggered a bit
      // flip to be sure we are trying a pattern that actually works when
      // replaying it
      Logger::log_info(
          "Hammering pattern with mapping to see how effective it is.");
      CodeJitter &jitter = it->get_code_jitter();
      triggered_bitflips = hammer_pattern(
          params, jitter, patt, *it, jitter.flushing_strategy,
          jitter.fencing_strategy, cur_reps, jitter.num_aggs_for_sync,
          jitter.total_activations, false, jitter.pattern_sync_each_ref, false,
          false, false, true, true, false);
    } else {
      Logger::log_info(
          "Using bit_flip data in JSON to determine effectiveness of mapping.");
      triggered_bitflips = it->count_bitflips();
    }

    Logger::log_success(
        format_string("Mapping triggered %d bit flips.", triggered_bitflips));

    if (triggered_bitflips > best_mapping_bitflips) {
      best_mapping_bitflips = triggered_bitflips;
      best_reproducibility_score =
          ReplayingHammerer::last_reproducibility_score;
      best_mapping = it;
    }
  }

  // e.g., if we run this on another DIMM, none of the mappings may work at the
  // same location (same mapping)
  if (best_mapping_bitflips == 0) {
    Logger::log_info("None of the existing mappings triggered a bit flip. "
                     "Choosing the first mapping.");
    return patt.address_mappings.front();
  }

  Logger::log_info(format_string("Best mapping (based on #bitflips): %s.",
                                 best_mapping->get_instance_id().c_str()));
  if (optimize_hammering_num_reps) {
    // e.g., if reproducibility_score = 0.1, i.e., we trigger a bit flips in
    // 1/10 times, we compute (1/0.1) * 1.2 = 12 as the number of repetitions we
    // need to do to trigger the bit flip
    hammering_num_reps = (best_reproducibility_score > 0)
                             ? static_cast<int>(std::ceil(
                                   (1.0f / best_reproducibility_score) * 2.0))
                             : initial_hammering_num_reps;
    Logger::log_info(format_string(
        "Based on the reproducibility, we set hammering_num_reps to %d.",
        hammering_num_reps));
  }
...
void ReplayingHammerer::replay_patterns(
    const std::string &json_filename,
    const std::unordered_set<std::string> &pattern_ids, bool e2e) {
  ...
  // mapping from pattern ID to number of bit flips of the most effective
  // mapping
  ...
  for (auto &patt : loaded_patterns) {
    ...
    PatternAddressMapper &mapper =
        determine_most_effective_mapping(patt, false, true);
    pattern_id_to_bitflips.insert(
        std::make_pair(patt.instance_id, mapper.count_bitflips()));
    pattern_id_to_length.insert(
        std::make_pair(patt.instance_id, patt.total_activations));

    // initialize the FuzzingParameterSet by extracting the data from the
    // pattern and mapping
    ...
    Logger::log_highlight(format_string(
        "Analyzing pattern %s using mapping %s:", patt.instance_id.c_str(),
        mapper.get_instance_id().c_str()));

    // :::::::::::::::::::::::::::::::::::::::::::::::::::::
    // ::: REPRODUCIBILITY
    // ::: In this experiment we determine how long it takes to retrigger bit
    // flips for a given pattern 10 times.
    // ::: We report how often we needed to retry the pattern and how long it
    // takes to trigger them on average (time).
    // :::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    std::vector<volatile char *> random_rows =
        mapper.get_random_nonaccessed_rows(params.get_max_row_no());
    ...
    while (rounds_with_bitflips++ < ROUNDS_WITH_BITFLIPS_GOAL) {
      ...
      while (cur_try++ < MAX_RETRIES && !success) {
        ...
        auto num_bitflips = hammer_pattern(
            params, jitter, patt, mapper, jitter.flushing_strategy,
            jitter.fencing_strategy, 1, jitter.num_aggs_for_sync,
            jitter.total_activations, false, jitter.pattern_sync_each_ref,
            false, false, false, false, true, e2e);
    ...
    repeatability_data.insert(
        std::make_pair(mapper.get_instance_id(), std::move(rep_data)));

    processed_patterns++;
    if (processed_patterns >= REPEATABILITY_MAX_NUM_PATTERNS)
      break;
...
size_t ReplayingHammerer::replay_patterns_brief(
    const std::string &json_filename,
    const std::unordered_set<std::string> &pattern_ids, size_t sweep_bytes,
    bool running_on_original_dimm, bool e2e) {
  auto patterns = load_patterns_from_json(json_filename, pattern_ids);
  return replay_patterns_brief(patterns, sweep_bytes, 1,
                               running_on_original_dimm, e2e);
}

size_t ReplayingHammerer::replay_patterns_brief(
    std::vector<HammeringPattern> hammering_patterns, size_t sweep_bytes,
  ...
  for (auto &pattern : hammering_patterns) {
    ...
    // check the HammeringPattern's data for the most effective pattern instead
    // of rerunning
    PatternAddressMapper &mapper =
        (running_on_original_dimm)
            ? pattern.get_most_effective_mapping()
            : determine_most_effective_mapping(pattern, false, false);

    Logger::log_highlight(format_string(
        "Sweeping pattern %s (%d/%d) using mapping %s",
        pattern.instance_id.c_str(), pattern_index++, hammering_patterns.size(),
        mapper.get_instance_id().c_str()));
    Logger::log_timestamp();

    // this is important to fill up the FuzzingParameterSet with the correct
    // values as this object is not exported 1:1 as object in the JSON file
    ...
    for (size_t i = 0; i < num_locations; ++i) {
      ...
      struct SweepSummary summary =
          sweep_pattern(pattern, mapper, (e2e ? 1 : 10), sweep_bytes,
                        direct_effective_aggs, e2e);
      ...
      if (i + 1 < num_locations) {
        // move pattern to another location and then continue sweeping from
        // there (we don't do this at the beginning of the for loop because we
        // want to include the sweep that starts at the start location of the
        // best mapping
  ...
  filename << "sweep-summary-" << num_locations << "x"
           << sweep_bytes / 1024 / 1024 << "MB.json";
...
std::vector<HammeringPattern> ReplayingHammerer::load_patterns_from_json(
    const std::string &json_filename,
  ...
  // this is for downwards-compatibility with the old JSON format where we just
  // had a JSON array of HammeringPatterns, i.e., without a separate 'metadata'
  // section
#ifdef ENABLE_JSON
  nlohmann::json patterns_array;
  patterns_array = json_file.contains("hammering_patterns")
                       ? json_file["hammering_patterns"]
                       : json_file;

  // if pattern_ids == nullptr: user did not provide --replay_patterns program
  // arg
  if (pattern_ids.empty()) {
    ...
    for (auto const &json_hammering_patt : patterns_array) {
      ...
      // we load all patterns; as the JSON contains only the patterns that
      // triggered bit flips, they all are effective because HammeringPattern
      // does not implement a copy constructor, we use the move operator here
      patterns.push_back(std::move(pattern));
      // attention: from here on the pattern does not exist anymore as it is
      // moved
    }
    // store the mapping: mapping ID -> hammering pattern, for each mapping of
    // the best pattern
    ...
    for (auto const &json_hammering_patt : patterns_array) {
      HammeringPattern pattern;
      from_json(json_hammering_patt, pattern);
      // after parsing, check if this pattern's ID matches one of the IDs given
      // to '-replay_patterns' Note: Due to a bug in the implementation, old
      // raw_data.json files may contain multiple HammeringPatterns with the
      // same ID and the exact same pattern but a different mapping. In this
      // case, we load ALL such patterns.
      if (pattern_ids.count(pattern.instance_id) > 0) {
        Logger::log_info(format_string("Found pattern %s and assoc. mappings:",
...
#else
  Logger::log_failure("Replaying mode requires ENABLE_JSON (see "
                      "CMakeLists.txt) to work. Cannot continue.");
  exit(EXIT_FAILURE);
#endif
...
size_t ReplayingHammerer::hammer_pattern(
    FuzzingParameterSet &fuzz_params, CodeJitter &code_jitter,
    ...
    unsigned long num_reps, int aggressors_for_sync, int num_activations,
    ...
    bool check_flips_after_each_rep, bool e2e) {

  // transform pattern into vector of addresses
  std::vector<volatile char *> hammering_accesses_vec;
  mapper.export_pattern(pattern.aggressors, pattern.base_period,
                        hammering_accesses_vec);

  return hammer_pattern(
      fuzz_params, code_jitter, pattern, mapper, flushing_strategy,
      fencing_strategy, num_reps, aggressors_for_sync, num_activations,
      early_stopping, sync_each_ref, verbose_sync, verbose_memcheck,
      verbose_params, wait_before_hammering, check_flips_after_each_rep, e2e,
      hammering_accesses_vec);
}
...
size_t ReplayingHammerer::hammer_pattern(
  ...
  // early_stopping: stop after the first repetition in that we observe any bit
  // flips
  ...
  auto const acts_per_tref = static_cast<int>(pattern.total_activations /
                                              pattern.num_refresh_intervals);
  code_jitter.jit_strict(acts_per_tref, flushing_strategy, fencing_strategy,
                         hammering_accesses_vec, sync_each_ref,
                         aggressors_for_sync, num_activations);

  // dirty hack to get correct output of flipped rows as we need to aggregate
  // the results over all tries
  std::vector<BitFlip> flipped_bits_acc;

  // note: we start counting the num_tries by 1, otherwise
  // reps_with_bitflips/num_tries may cause a division-by-zero
  size_t num_tries = 1;
  for (; num_tries <= num_reps; num_tries++) {
    // TODO: Analyze using Hynix whether this waiting really makes sense,
    // otherwise remove it wait a specific time while doing some random accesses
    // before starting hammering
    auto wait_until_hammering_us =
        fuzz_params.get_random_wait_until_start_hammering_us();
    ...
    if (wait_before_hammering && wait_until_hammering_us > 0) {
      random_rows =
          mapper.get_random_nonaccessed_rows(fuzz_params.get_max_row_no());
    ...
    // check for bit flips if check_flips_after_each_rep=true or if we're in the
    // last iteration
    if (check_flips_after_each_rep || num_tries == num_reps - 1) {
      // check if any bit flips happened
      // it's important that we run in reproducibility mode, otherwise the bit
      // flips vec in the mapping is changed!
      ...
      flipped_bits_acc.insert(flipped_bits_acc.end(), mem.flipped_bits.begin(),
                              mem.flipped_bits.end());

      if (e2e && !mem.flipped_bits.empty()) {
        Rubicon rubicon(mem.use_page, mem.use_hugepage);
        rubicon.e2e(code_jitter, fuzz_params, hammering_accesses_vec, random_rows, wait_until_hammering_us, mem.flipped_bits);
      }

      // in early_stopping mode, we do not carry out all repetitions but stop
      // after we have found at least one bit flip
      if (num_bitflips > 0 && early_stopping)
        break;
    }
  }

  ReplayingHammerer::last_reproducibility_score =
      (int)std::ceil(reps_with_bitflips / num_tries);
...
struct SweepSummary
ReplayingHammerer::sweep_pattern(HammeringPattern &pattern,
                                 PatternAddressMapper &mapper, size_t num_reps,
                                 size_t size_bytes) {
  return sweep_pattern(pattern, mapper, num_reps, size_bytes, {}, false);
}

struct SweepSummary ReplayingHammerer::sweep_pattern(
    HammeringPattern &pattern, PatternAddressMapper &mapper, size_t num_reps,
    size_t size_bytes,
    const std::unordered_set<AggressorAccessPattern> &effective_aggs,
    bool e2e) {
  // sweep_pattern modifies the original mapping, thus we create a copy and
  // restore it again before returning from func
  PatternAddressMapper original_mapping = mapper;

  // sweep over a chunk of N MBytes to see whether this pattern also works on
  // other locations compute the bound of the mem area we want to check using
  // this pattern
  ...
  Logger::log_info(format_string(
      "Sweeping pattern %s with mapping %s over %d MB, equiv. to %d rows, with "
      "each %d repetitions.",
  ...
  for (unsigned long r = 1; r <= num_rows; ++r) {
    ...
    // modify assignment of agg ID to DRAM address by shifting rows of all
    // aggressors
    shift =
        nonconsec
            ? Range<size_t>(0, params.get_max_row_no()).get_random_number(gen)
            : 1;
    offset += shift;
    offset = offset % params.get_max_row_no();
    mapper.shift_mapping(params, shift, effective_aggs);

    // call hammer_pattern
    auto num_flips = hammer_pattern(
        params, jitter, pattern, mapper, jitter.flushing_strategy,
        jitter.fencing_strategy, num_reps, jitter.num_aggs_for_sync,
        jitter.total_activations, true, jitter.pattern_sync_each_ref, false,
        false, false, true, true, e2e);
    // note the use of early_stopping in hammer_pattern: we repeat hammering at
    // maximum hammering_num_reps times but do stop after observing any bit flip
    // - this is the number that we report
    total_bit_flips_sweeping += num_flips;
    bflips.insert(bflips.end(), mem.flipped_bits.begin(),
                  mem.flipped_bits.end());
    for (auto bf : mem.flipped_bits) {
      bitflips_list.push_back(bf);
    }
    // for (auto bf : mem.flipped_bits) std::cout << bf.address.to_virt() << " "
  ...
  Logger::log_data(
      format_string("Total corruptions: %ld", total_bit_flips_sweeping));
  ...
  struct SweepSummary sweepsum = {.num_flips_z2o = z2o_corruptions,
                                  .num_flips_o2z = o2z_corruptions,
                                  .observed_bitflips = bitflips_list};
  return sweepsum;
}

ReplayingHammerer::ReplayingHammerer(Memory &mem, bool nonconsec)
    : mem(mem), nonconsec(nonconsec) { /* NOLINT */
  std::random_device rd;
  gen = std::mt19937(rd());
}

[[maybe_unused]] void ReplayingHammerer::run_refresh_alignment_experiment(
    PatternAddressMapper &mapper) {
  ...
  size_t num_bit_flips = hammer_pattern(
      params, cj, patt, mapper, cj.flushing_strategy, cj.fencing_strategy, 10,
      cj.num_aggs_for_sync, M(100), false, cj.pattern_sync_each_ref, false,
  ...
  num_bit_flips = hammer_pattern(params, cj, patt, mapper, cj.flushing_strategy,
                                 cj.fencing_strategy, 10, cj.num_aggs_for_sync,
                                 M(10), false, cj.pattern_sync_each_ref, false,
                                 false, false, false, true, false);
  Logger::log_data(format_string("total bit flips = %d", num_bit_flips));
}

[[maybe_unused]] void
ReplayingHammerer::run_code_jitting_probing(PatternAddressMapper &mapper) {
  auto &cj = mapper.get_code_jitter();
  auto &patt = map_mapping_id_to_pattern.at(mapper.get_instance_id());

  // - FLUSHING_STRATEGY / FENCING_STRATEGY
  //      for (const auto &ff_strategy : get_valid_strategies()) {
  //        auto num_bit_flips = hammer_pattern(mem, params, jitter, patt,
  //        mapper, ff_strategy.first, ff_strategy.second,
  //            hammering_num_reps, sync_each_ref, num_sync_aggs, total_acts,
  //            false, verbose_memcheck, verbose_params);
  //        Logger::log_info(format_string("FLUSHING_STRATEGY = %-17s,
  //        FENCING_STRATEGY = %-19s => %d bit flips",
  //            to_string(ff_strategy.first).c_str(),
  //            to_string(ff_strategy.second).c_str(), num_bit_flips));
  //      }

  // - sync_each_ref
  for (auto &sync : {true, false}) {
    auto num_bit_flips = hammer_pattern(
        params, cj, patt, mapper, cj.flushing_strategy, cj.fencing_strategy,
        hammering_num_reps, cj.num_aggs_for_sync, cj.total_activations, false,
        sync, false, false, false, true, true, false);
  ...
  for (const auto &sync_aggs : {1, 2}) {
    auto num_bit_flips = hammer_pattern(
        params, cj, patt, mapper, cj.flushing_strategy, cj.fencing_strategy,
        hammering_num_reps, sync_aggs, cj.total_activations, false,
        cj.pattern_sync_each_ref, false, false, false, true, true, false);
    Logger::log_info(format_string("num_aggs_for_sync = %-7d => %d bit flips",
                                   sync_aggs, num_bit_flips));
  }
  ...
  for (int a = M(10); a > M(1); a -= M(1)) {
    auto num_bit_flips = hammer_pattern(
        params, cj, patt, mapper, cj.flushing_strategy, cj.fencing_strategy,
        hammering_num_reps, cj.num_aggs_for_sync, a, false,
        cj.pattern_sync_each_ref, false, false, false, true, true, false);
    Logger::log_info(format_string(
        "total_num_acts_hammering = %-12d => %d bit flips", a, num_bit_flips));
  }
}
...
void ReplayingHammerer::find_direct_effective_aggs(
    HammeringPattern &pattern, PatternAddressMapper &mapper,
    std::unordered_set<AggressorAccessPattern> &direct_effective_aggs) {
  // we consider an aggressor as being responsible for a bit flip if it is at
  // maximum DIST_THRESHOLD rows away from the flipped row
  ...
  if (mapper.bit_flips.size() == 0)
    return;
  for (const auto &bf : mapper.bit_flips.at(0))
    flipped_rows.insert(static_cast<int>(bf.address.row));

  // map to keep track of best guess which AggressorAccessPattern caused a bit
  // flip. maps (aggressor ID) to (distance, AggressorAccessPattern)
  std::unordered_map<int, std::pair<int, AggressorAccessPattern>> matches;

  // iterate over all (unique) bit flips and compute the distance to all
  // AggressorAccessPatterns, remember the AggressorAccessPattern with the
  // lowest distance - this is probably the one that triggered the bit flip
  // TODO: handle the case where multiple aggressors have the same distance to
  // the flipped row
  // TODO: add threshold, e.g., if smallest distance of AggressorAccessPatterns
  // and flipped row is larger than
  //  X rows, there's probably remapping going on and we cannot tell which
  //  AggressorAccessPattern caused the flip
  ...
  for (const auto &flipped_row : flipped_rows) {
    for (auto &agg_pair : pattern.agg_access_patterns) {
      // take the smaller distance of the aggressors as the distance of the
      // aggressor pair
      int min_distance = std::numeric_limits<int>::max();
      for (const auto &agg : agg_pair.aggressors) {
        // measure distance between aggs in agg pairs and flipped bit
        auto cur_distance =
            std::abs((int)cur_mapping.at(agg.id).row - flipped_row);
        min_distance = std::min(min_distance, cur_distance);
      }
      // check if we don't have any 'best' candidate for this bit flip yet (a)
      // or whether this distance is lower than the distance of the best
      // candidate found so far (b)
      if (matches.count(flipped_row) == 0 // (a)
          || (matches.count(flipped_row) > 0 &&
              matches.at(flipped_row).first > min_distance)) { // (b)
  ...
  // direct_effective_aggs contains an AggressorAccessPattern for each flipped
  // row, that's the pattern with the lowest distance to the flipped row
  for (const auto &fr : matches) {
    // if the distance to the flipped row is larger than the defined threshold,
    // we don't consider the aggressor as being the one who triggered the bit
    // flip as probably remapping is going on
    if (fr.second.first > DIST_THRESHOLD) {
      Logger::log_info(format_string(
          "Aggressor %d with distance %d from flipped row is too far away for "
          "being considered as effective. Skipping it.",
    ...
    for (auto &agg : fr.second.second.aggressors) {
      ...
      if (agg.id != fr.second.second.aggressors.rbegin()->id)
        sstream << ",";
    }
    sstream << "]";
    Logger::log_data(format_string("Flip in row %d, probably caused by [%s] -> "
                                   "%s due to minimum distance %d.",
                                   fr.first,
                                   fr.second.second.to_string().c_str(),
                                   sstream.str().c_str(), fr.second.first));
...
void ReplayingHammerer::find_direct_effective_aggs(
  ...
  if (map_mapping_id_to_pattern.count(mapper.get_instance_id()) > 0) {
    Logger::log_error("find_direct_effective_aggs: Could not find pattern "
                      "associated with given mapping. Aborting.");
    exit(EXIT_FAILURE);
  }
...
[[maybe_unused]] void ReplayingHammerer::find_indirect_effective_aggs(
    PatternAddressMapper &mapper,
    const std::unordered_set<AggressorAccessPattern> &direct_effective_aaps,
    std::unordered_set<AggressorAccessPattern> &indirect_effective_aggs) {
  HammeringPattern &patt =
      map_mapping_id_to_pattern.at(mapper.get_instance_id());
  CodeJitter &jitter = *mapper.code_jitter;

  // change temporal access patterns: randomize mapping of
  // AggressorAccessPatterns, one-by-one, to see after which changes the pattern
  // still triggers bit flips
  auto &cur_mapping = mapper.aggressor_to_addr;
  for (const auto &agg_pair : patt.agg_access_patterns) {
    // if this is the aggressor access pattern that caused the bit flip: skip it
    if (direct_effective_aaps.count(agg_pair) > 0)
      continue;

    // store old location of aggressors in this aggressor access pattern
    std::unordered_map<int, DRAMAddr> old_mappings;
    // store the lowest row number of the aggressors as we will later need it to
    // compute its new row
    ...
    // randomize row of this aggressor by mapping it to a row outside of the
    // [min,max] area of the current pattern
    auto max_row = params.get_max_row_no();
    auto offset = (mapper.max_row - lowest_row_no +
                   Range<int>(1, 256).get_random_number(gen)) %
                  max_row;
    for (const auto &agg : agg_pair.aggressors) {
    ...
    size_t num_triggered_bitflips = hammer_pattern(
        params, jitter, patt, mapper, jitter.flushing_strategy,
        jitter.fencing_strategy, hammering_num_reps, jitter.num_aggs_for_sync,
        jitter.total_activations, false, jitter.pattern_sync_each_ref, false,
        false, false, true, true, false);

    // check if pattern still triggers bit flip to see whether this
    // AggressorAccessPattern matters
    if (num_triggered_bitflips > 0) {
      Logger::log_info(format_string(
          "Shifting agg pair [%s] by %d rows.\n"
      ...
      // mark this AggressorAccessPattern as effective/essential for trigger bit
      // flips
      indirect_effective_aggs.insert(agg_pair);
      // restore the original mapping as this AggressorAccessPattern matters!
      for (const auto &agg : agg_pair.aggressors)
        cur_mapping[agg.id] = old_mappings[agg.id];
    }
  }
  Logger::log_info(
      "Mapping after randomizing all non-effective aggressor pairs:");
  Logger::log_data(mapper.get_mapping_text_repr());
}

[[maybe_unused]] void ReplayingHammerer::run_pattern_params_probing(
    PatternAddressMapper &mapper,
    const std::unordered_set<AggressorAccessPattern> &direct_effective_aggs,
    std::unordered_set<AggressorAccessPattern> &indirect_effective_aggs) {
  // technique: change temporal props, which decide how the pattern is accessed,
  // only for agg(s) that triggered bit flip systematically test combinations of
  // (amplitude, frequency, phase) for this aggressor access pattern

  // transform std::unordered_set<AggressorAccessPattern> into
  // std::vector<AggressorAccessPattern> because std::unordered_set only
  // provides const iterators that do not allow modifying the element's
  // attributes
  ...
  auto sort_by_idx_dist = [](std::vector<int> &vec, size_t idx) {
    ...
    for (int i = 1; i < static_cast<int>(num_elements); ++i) {
      if (index + i < static_cast<int>(num_elements))
  ...
  auto get_index = [](const std::vector<int> &vec, int elem) -> size_t {
    ...
    if (it != vec.end()) {
      ...
      Logger::log_error(
          format_string("ReplayingHammerer.cpp:get_index(...) could not find "
                        "given element (%d) in vector (%s).",
                        elem, ss.str().c_str()));
      exit(EXIT_FAILURE);
    }
  };
  ...
  std::vector<int> allowed_frequencies =
      PatternBuilder::get_available_multiplicators(
          (int)(patt.total_activations / (size_t)base_period));

  size_t max_trials_per_aap = 48;
  const auto fpa_probing_num_reps = 5;

  // The strategy that we use here is motivated by the following:
  // Given a AggressorAccessPattern AAP(freq,phase,amplitude)=(f,p,a) that
  // triggers a bit flip, we cannot try out all possible combinations of freq,
  // phase, and amplitude as it simply would take too long. We could just try
  // out randomly chosen combinations but, well, we already did this to find
  // that pattern. Instead, we choose the same (f,p,a) as we know that triggers
  // bit flips and then slowly change each of these parameters to see how it
  ...
  Logger::log_info("Systematically testing different combination of "
                   "(frequency, phase, amplitude).");
  // note: usually, a pattern only causes a bit flip in one row and hence
  // direct_effective_aggs has one element only
  for (AggressorAccessPattern &aap : direct_effective_aggs_vec) {
    size_t cnt = 0;

    const int orig_frequency = static_cast<int>(aap.frequency);
    const auto orig_frequency_idx =
        get_index(allowed_frequencies, orig_frequency / patt.base_period);
    ...
    for (int i = 0; i <= base_period - num_aggressors; ++i)
      allowed_phases.push_back(i);

    const auto orig_start_offset = static_cast<int>(aap.start_offset);
    const auto orig_start_offset_idx =
        get_index(allowed_phases, orig_start_offset);
    sort_by_idx_dist(allowed_phases, orig_start_offset_idx);

    Logger::log_highlight(format_string(
        "Original parameters (freq, ph, amp) = (%d, %d, %d)",
    ...
    for (const auto &freq : allowed_frequencies) {
      ...
      for (const auto &phase : allowed_phases) {
        ...
        for (auto i = 1; (i * num_aggressors) <= base_period - phase; ++i)
          allowed_amplitudes.push_back(i);

        const auto orig_amplitude_idx =
            get_index(allowed_amplitudes, orig_amplitude);
        ...
        for (const auto &amplitude : allowed_amplitudes) {
          ...
          std::vector<AggressorAccessPattern> aggs(
              indirect_effective_aggs.begin(), indirect_effective_aggs.end());
          ...
          Logger::log_highlight(format_string(
              "ROUND %d | Parameters (freq, ph, amp) = (%d, %d, %d)", cnt, freq,
              phase, amplitude));

          // back up the original AggressorAccessPatterns by moving them
          std::vector<AggressorAccessPattern> old_aaps =
              std::move(patt.agg_access_patterns);

          // fill up the slots in the pattern
          builder.prefill_pattern(patt.total_activations, aggs);

          // NOTE: We don't include the FuzzingParameterSet that found the
          // HammeringPattern in the JSON yet, so
          //  passing a new FuzzingParameterSet instance 'params' here could
          //  actually break things, e.g., if parameter ranges are too narrow
          //  and we cannot fill up the remaining slots
          builder.generate_frequency_based_pattern(
              params, patt.total_activations, patt.base_period);

          // as we changed the AggressorAccessPattern, the existing Agg ID ->
          // DRAM Address mapping is not valid anymore and we need to generate a
          // new mapping
          // TODO: Maybe we should randomize fpa_probing_num_reps times to be
          // sure that it just doesn't work because
          //  we are hammering at a unfavourable location? Alternatively, we
          //  could let the direct effective AggressorAccessPatterntarget always
          //  a target known-to-be vulnerable row so we can be sure that if we
          ...
          auto num_bitflips = ReplayingHammerer::hammer_pattern(
              params, jitter, patt, mapper, jitter.flushing_strategy,
              jitter.fencing_strategy, fpa_probing_num_reps,
              jitter.num_aggs_for_sync, jitter.total_activations, false,
          ...
          if (num_bitflips == 0) {
            ...
            Logger::log_success(
                format_string("Found %lu bit flips, on average %2.f per "
                              "hammering rep (%d). Flipped row(s): %s.",
                              num_bitflips,
                              static_cast<float>(num_bitflips) /
                                  static_cast<float>(fpa_probing_num_reps),
          ...
          if (cnt % max_trials_per_aap == 0)
            goto continue_next;
        }
      }
    }
  continue_next:
    Logger::log_info("Threshold for number of analysis runs reached, "
                     "continuing with next AggressorAccessPattern.");
  }
}
...
void ReplayingHammerer::derive_FuzzingParameterSet_values(
  ...
  // as we always choose the number of activations of a pattern in a way that it
  // is a multiple of the number of activations within a refresh interval, we
  // can use these two values to reconstruct the num activations per tREFI that
  // we assumed/measured (experimentally determined)
  params = FuzzingParameterSet(static_cast<int>(pattern.total_activations /
                                                pattern.num_refresh_intervals));
  ...
  // cannot be restored (sync_each_ref is a Range): must be passed from
  // CodeJitter when calling hammer_pattern
  //  params.sync_each_ref = jitter.pattern_sync_each_ref;

  int agg_intra_dist, agg_inter_dist;
  bool use_seq_addrs = false;
  mapper.compute_mapping_stats(pattern.agg_access_patterns, agg_intra_dist,
                               agg_inter_dist, use_seq_addrs);

  params.set_use_sequential_aggressors(
      Range<int>(use_seq_addrs, use_seq_addrs));
  params.set_agg_inter_distance(agg_inter_dist);
  params.set_agg_intra_distance(agg_intra_dist);

  // Note: The JSON that we export includes most but not all parameters. Some
  // parameters are defined via ranges and not included in the JSON. When
  // generating a new FuzzingParameterSet, the ranges currently defined in the
  // FuzzingParameterSet are used instead. This should be fine as it only
  // includes less important parameters. The parameters used by the
  // ReplayingHammerer that cannot be restored from the JSON are:
