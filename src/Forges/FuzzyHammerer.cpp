...
void FuzzyHammerer::n_sided_frequency_based_hammering(DramAnalyzer &dramAnalyzer, Memory &memory, int acts,
  ...
  ReplayingHammerer replaying_hammerer(memory, nonconsec);
...
//
      ...
      sum_flips_one_pattern_all_mappings += probe_mapping_and_scan(mapper, memory, fuzzing_params, program_args.num_dram_locations_per_mapping, e2e);
  ...
  for (auto &patt : effective_patterns) {
    for (auto &mapping : patt.address_mappings) {
      ...
      // do the minisweep
      SweepSummary summary = replaying_hammerer.sweep_pattern(patt, mapping, 10, SWEEP_MEM_SIZE_BEST_PATTERN, {}, false);
...
void FuzzyHammerer::test_location_dependence(ReplayingHammerer &rh, HammeringPattern &pattern) {
  ...
  SweepSummary ss_move_selected = rh.sweep_pattern(pattern, best_mapping, 1, MB(8), direct_effective_aggs, false);
...
size_t FuzzyHammerer::probe_mapping_and_scan(PatternAddressMapper &mapper, Memory &memory,
  ...
  for (size_t dram_location = 0; dram_location < num_dram_locations; ++dram_location) {
    ...
    if (e2e) {
      Rubicon rubicon(memory.use_page, memory.use_hugepage);
      rubicon.e2e(code_jitter, fuzzing_params, hammering_accesses_vec, random_rows, wait_until_hammering_us, mapper.bit_flips.back());
      return flipped_bits;
    }

    // now shift the mapping to another location
    std::mt19937 gen = std::mt19937(std::random_device()());
    mapper.shift_mapping(fuzzing_params, Range<int>(1,32).get_random_number(gen), {});
