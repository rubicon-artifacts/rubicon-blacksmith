...
void DramAnalyzer::find_bank_conflicts() {
  ...
  int remaining_tries =
      NUM_BANKS * 256; // experimentally determined, may be unprecise
  ...
  reset:
    ...
    if (remaining_tries == 0) {
      Logger::log_error(
          format_string("Could not find conflicting address sets. Is the "
                        "number of banks (%d) defined correctly?",
                        (int)NUM_BANKS));
      exit(1);
    }
  }
...
void DramAnalyzer::find_targets(std::vector<volatile char *> &target_bank) {
  // create an unordered set of the addresses in the target bank for a quick
  // lookup std::unordered_set<volatile char*> tmp;
  // tmp.insert(target_bank.begin(), target_bank.end());
  std::unordered_set<volatile char *> tmp(target_bank.begin(),
                                          target_bank.end());
  target_bank.clear();
...
DramAnalyzer::DramAnalyzer(volatile char *target)
  ...
  banks = std::vector<std::vector<volatile char *>>(
      NUM_BANKS, std::vector<volatile char *>());
}
...
void DramAnalyzer::load_known_functions(int num_ranks) {
  if (num_ranks == 1) {
    bank_rank_functions =
        std::vector<uint64_t>({0x2040, 0x24000, 0x48000, 0x90000});
    row_function = 0x3ffe0000;
  } else if (num_ranks == 2) {
    bank_rank_functions =
        std::vector<uint64_t>({0x2040, 0x44000, 0x88000, 0x110000, 0x220000});
    row_function = 0x3ffc0000;
  } else {
    Logger::log_error(
        "Cannot load bank/rank and row function if num_ranks is not 1 or 2.");
    exit(1);
  }
...
size_t DramAnalyzer::count_acts_per_ref() {
  ...
  auto compute_std = [](std::vector<uint64_t> &values, uint64_t running_sum,
                        size_t num_numbers) {
    double mean =
        static_cast<double>(running_sum) / static_cast<double>(num_numbers);
    double var = 0;
  ...
  for (size_t i = 0;; i++) {
    ...
    if ((after - before) > 1000) {
      if (i > skip_first_N && count_old != 0) {
        ...
        running_sum += value;
        // check after each 200 data points if our standard deviation reached 1
        // -> then stop collecting measurements
        if ((acts.size() % 200) == 0 &&
            compute_std(acts, running_sum, acts.size()) < 3.0)
          break;
      }
      count_old = count;
    }
