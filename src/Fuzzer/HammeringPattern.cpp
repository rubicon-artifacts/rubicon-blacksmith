...
void from_json(const nlohmann::json &j, HammeringPattern &p) {
  ...
  j.at("agg_access_patterns")
      .get_to<std::vector<AggressorAccessPattern>>(p.agg_access_patterns);
  j.at("address_mappings")
      .get_to<std::vector<PatternAddressMapper>>(p.address_mappings);
}

#endif

HammeringPattern::HammeringPattern(int base_period)
    : instance_id(uuid::gen_uuid()), base_period(base_period), max_period(0),
      total_activations(0), num_refresh_intervals(0),
      is_location_dependent(false) {}

HammeringPattern::HammeringPattern()
    : instance_id(uuid::gen_uuid()), base_period(0), max_period(0),
      total_activations(0), num_refresh_intervals(0),
      is_location_dependent(false) {}
...
std::string HammeringPattern::get_pattern_text_repr() {
  std::stringstream ss;
  // depending on the number of aggressors, decide how many digits to use to
  // represent each aggressor ID we assume that if we have more than two
  // AggressorAccessPatterns, then it is fully filled pattern and we can just
  // use the number of aggressors as a way to determine how many digits we need,
  // otherwise it's probably a empty/prefilled pattern and we assume 2 digits
  // (as ID_PLACEHOLDER_AGG is -1)
  auto dwidth =
      (agg_access_patterns.size() > 2) ? get_num_digits(aggressors.size()) : 2;
  for (size_t i = 0; i < aggressors.size(); ++i) {
    // add a new line after each base period to make it easier to check a
    // pattern's correctness
    if ((i % base_period) == 0 && i > 0)
      ss << std::endl;
...
std::string HammeringPattern::get_agg_access_pairs_text_repr() {
  ...
  for (const auto &agg_acc_pair : agg_access_patterns) {
    // add a new line after each three aggressor access patterns to avoid
    // unintended text wrapping in terminal
    if (cnt > 0 && cnt % 3 == 0)
      ss << std::endl;
    ss << std::setw(30) << std::setfill(' ') << std::left
       << agg_acc_pair.to_string();
    cnt++;
  }
  return ss.str();
}
...
HammeringPattern::get_access_pattern_by_aggressor(Aggressor &agg) {
  // iterate over the AggressorAccessPatterns and return the *first*
  // AggressorAccessPattern that has the given Aggressor agg as its *first*
  // Aggressor
  for (auto &aap : agg_access_patterns) {
    if (aap.aggressors[0].id == agg.id)
      return aap;
  }
...
PatternAddressMapper &HammeringPattern::get_most_effective_mapping() {
  if (address_mappings.empty()) {
    Logger::log_error(
        "get_most_effective_mapping() failed: No mappings existing!");
    exit(EXIT_FAILURE);
  }
