...
void PatternAddressMapper::randomize_addresses(
  ...
  // retrieve and then store randomized values as they should be the same for
  // all added addresses (store bank_no as field for
  // get_random_nonaccessed_rows)
  bank_no = PatternAddressMapper::bank_counter;
  PatternAddressMapper::bank_counter =
      (PatternAddressMapper::bank_counter + 1) % NUM_BANKS;
  ...
  // we can make use here of the fact that each aggressor (identified by its ID)
  // has a fixed N, that means, is either accessed individually (N=1) or in a
  // group of multiple aggressors (N>1; e.g., N=2 for double sided)
  // => if we already know the address of any aggressor in an aggressor access
  // pattern, we already must know addresses for all of them as we must have
  // accessed all of them together before
  size_t row;
  ...
  Logger::log_info(
      format_string("[PatternAddressMapper] Target no. of DRAM rows = %d",
  ...
  // probability to map aggressor to same row as another aggressor is already
  // mapped to
  const int prob2 =
      100 -
      (static_cast<int>(
          std::min(static_cast<double>(fuzzing_params.get_num_aggressors()) /
                       static_cast<double>(total_abstract_aggs),
                   1.0) *
          100));
  Logger::log_info(format_string("[PatternAddressMapper] Probability to map "
                                 "multiple AAPs to same DRAM row = %d",
                                 prob2));
  ...
  std::discrete_distribution<> dist(weights.begin(),
                                    weights.end()); // Create the distribution
  ...
  //  Logger::log_info("Generating 1k random numbers to see how well
  //  distribution works "); size_t cnt_0 = 0; size_t cnt_1 = 0; for (size_t i =
  //  0; i < 1000; ++i) {
  //    if (dist(engine) == 0)
  //      cnt_0++;
  //    else
  ...
  for (auto &acc_pattern : agg_access_patterns) {
    for (size_t i = 0; i < acc_pattern.aggressors.size(); i++) {
      ...
      if (aggressor_to_addr.count(current_agg.id) > 0) {
        ...
        auto last_addr =
            aggressor_to_addr.at(acc_pattern.aggressors.at(i - 1).id);
        // update cur_row for its next use (note that here it is: cur_row =
        // last_addr.row)
        cur_row =
        ...
        // if use_seq_addresses is true, we use the last address and add the
        // agg_inter_distance on top -> this is the
        //   row of the next aggressor
        // if use_seq_addresses is false, we just pick any random row no.
        // between [0, 8192]
        ...
        if (map_to_existing_agg && !occupied_rows.empty()) {
          auto idx =
              Range<size_t>(1, occupied_rows.size()).get_random_number(gen) - 1;
        ...
        retry:
          row = use_seq_addresses
                    ? cur_row
                    : (Range<size_t>(cur_row,
                                     cur_row + fuzzing_params.get_max_row_no())
                           .get_random_number(gen) %
                       fuzzing_params.get_max_row_no());

          // check that we haven't assigned this address yet to another
          // aggressor ID if use_seq_addresses is True, the only way that the
          // address is already assigned is that we already flipped around the
          // address range once (because of the modulo operator) so that
          ...
          if (!use_seq_addresses && occupied_rows.count(row) > 0) {
            ...
            if (assignment_trial_cnt < 7)
              goto retry;
            Logger::log_info(
                format_string("Assigning unique addresses for Aggressor ID %d "
                              "didn't succeed. Giving up after 3 trials.",
                              current_agg.id));
          }
        }
      }
      ...
      aggressor_to_addr.insert(std::make_pair(
          current_agg.id, DRAMAddr(static_cast<size_t>(bank_no), row, 0)));
    }
  }
  ...
  if (verbose)
    Logger::log_info(
        format_string("Found %d different aggressors (IDs) in pattern.",
                      aggressor_to_addr.size()));
}

void PatternAddressMapper::determine_victims(
    const std::vector<AggressorAccessPattern> &agg_access_patterns) {
    ...
    for (auto &agg : acc_pattern.aggressors) {
      ...
      if (aggressor_to_addr.count(agg.id) == 0) {
        Logger::log_error(format_string(
            "Could not find DRAMAddr mapping for Aggressor %d", agg.id));
        exit(EXIT_FAILURE);
      }

      const auto dram_addr = aggressor_to_addr.at(agg.id);

      for (int delta_nrows = -ROW_THRESHOLD; delta_nrows <= ROW_THRESHOLD;
           ++delta_nrows) {
        auto cur_row_candidate = static_cast<int>(dram_addr.row) + delta_nrows;

        // don't add the aggressor itself and ignore any non-existing (negative)
        // row no.
        ...
        auto victim_start =
            DRAMAddr(dram_addr.bank, static_cast<size_t>(cur_row_candidate), 0);
        if (victim_rows.count(
                static_cast<volatile char *>(victim_start.to_virt())) > 0)
          continue;

        victim_rows.insert(
            static_cast<volatile char *>(victim_start.to_virt()));
      }
    }
  }
}
...
void PatternAddressMapper::export_pattern_internal(
    ...
    std::vector<volatile char *> &addresses, std::vector<int> &rows) {
  ...
  for (size_t i = 0; i < aggressors.size(); ++i) {
    ...
    // check whether there exists a aggressor ID -> address mapping before
    // trying to access it
    if (aggressor_to_addr.count(agg.id) == 0) {
      Logger::log_error(format_string(
    ...
    // retrieve virtual address of current aggressor in pattern and add it to
    // output vector
    addresses.push_back(
        (volatile char *)aggressor_to_addr.at(agg.id).to_virt());
  ...
  if (invalid_aggs) {
    Logger::log_error("Found at least an invalid aggressor in the pattern. "
                      "These aggressors were NOT added but printed to "
...
void PatternAddressMapper::export_pattern(std::vector<Aggressor> &aggressors,
  ...
  if (max_rows < rows_vector.size()) {
    Logger::log_error("Exporting pattern failed! Given plain-C 'rows' array is "
                      "too small to hold all aggressors.");
  }
...
std::string PatternAddressMapper::get_mapping_text_repr() {
  // get all keys (this is to not assume that keys always must start by 1) and
  // sort them
  std::vector<int> keys;
  ...
  for (const auto &k : keys) {
    if (cnt > 0 && cnt % 3 == 0)
      mapping_str << std::endl;
    mapping_str << std::setw(3) << std::left << k << " -> " << std::setw(13)
                << std::left << aggressor_to_addr.at(k).to_string_compact()
                << "   ";
    cnt++;
  }
...
void to_json(nlohmann::json &j, const PatternAddressMapper &p) {
  if (p.code_jitter == nullptr) {
    Logger::log_error("CodeJitter is nullptr! Cannot serialize "
                      "PatternAddressMapper without causing segfault.");
    return;
  }
...
std::string &PatternAddressMapper::get_instance_id() { return instance_id; }

const std::unordered_set<volatile char *> &
PatternAddressMapper::get_victim_rows() const {
  return victim_rows;
}

std::vector<volatile char *>
PatternAddressMapper::get_random_nonaccessed_rows(int row_upper_bound) {
  // we don't mind if addresses are added multiple times
  std::vector<volatile char *> addresses;
  for (int i = 0; i < 1024; ++i) {
    auto row_no =
        Range<int>(max_row, max_row + min_row).get_random_number(gen) %
        row_upper_bound;
    addresses.push_back(static_cast<volatile char *>(
        DRAMAddr(static_cast<size_t>(bank_no), static_cast<size_t>(row_no), 0)
            .to_virt()));
  }
  return addresses;
}

void PatternAddressMapper::shift_mapping(
    FuzzingParameterSet &fuzzing_params, int rows,
    const std::unordered_set<AggressorAccessPattern> &aggs_to_move) {
  ...
  for (auto &agg_acc_patt : aggressor_to_addr) {
    // if aggs_to_move is empty, we consider it as 'move all aggressors';
    // otherwise we check whether the current aggressor ID is in aggs_to_move
    // prior shifting the aggressor by the given number of rows (param: rows)
    if (aggs_to_move.empty() || movable_ids.count(agg_acc_patt.first) > 0) {
      agg_acc_patt.second.row =
          (agg_acc_patt.second.row + rows) % fuzzing_params.get_max_row_no();
...
PatternAddressMapper::PatternAddressMapper(const PatternAddressMapper &other)
    : victim_rows(other.victim_rows), instance_id(other.instance_id),
      min_row(other.min_row), max_row(other.max_row), bank_no(other.bank_no),
      aggressor_to_addr(other.aggressor_to_addr), bit_flips(other.bit_flips),
  ...
  code_jitter->pattern_sync_each_ref =
      other.get_code_jitter().pattern_sync_each_ref;
  std::random_device rd;
  gen = std::mt19937(rd());
}
...
void PatternAddressMapper::compute_mapping_stats(
    std::vector<AggressorAccessPattern> &agg_access_patterns,
    int &agg_intra_distance, int &agg_inter_distance, bool uses_seq_addresses) {
  Logger::log_info("Deriving mapping parameters from AggressorAccessPatterns.");

  // find first AggressorAccessPattern with more than one aggressor, then
  // compute distance in-between aggressors
  agg_intra_distance = 0;
  for (auto &agg_access_pattern : agg_access_patterns) {
    if (agg_access_pattern.aggressors.size() > 1) {
      auto r1 =
          aggressor_to_addr.at(agg_access_pattern.aggressors.at(1).id).row;
      auto r0 =
          aggressor_to_addr.at(agg_access_pattern.aggressors.at(0).id).row;
      agg_intra_distance = static_cast<int>(r1 - r0);
      break;
    }
  }

  // if all consecutive AggressorAccessPatterns have the same inter-distance,
  // then they use "sequential addresses"
  uses_seq_addresses = true;
  agg_inter_distance = -1;
  for (auto it = agg_access_patterns.begin();
       it + 1 != agg_access_patterns.end(); ++it) {
    auto this_size = it->aggressors.size();
    auto this_row =
        aggressor_to_addr.at(it->aggressors.at(this_size - 1).id).row;
  ...
  Logger::log_data(format_string("use_seq_addresses = %s",
                                 (uses_seq_addresses ? "true" : "false")));
}
...
void PatternAddressMapper::remap_aggressors(DRAMAddr &new_location) {
  // determine the mapping with the smallest row no -- this is the start point
  // where we apply our new location on
  ...
  for (auto &[id, addr] : aggressor_to_addr) {
    ...
    // for the row, we need to shift accordingly to preserve the distances
    // between aggressors
    addr.row += offset;
  }
}
