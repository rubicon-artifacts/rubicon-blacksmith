...
CodeJitter::CodeJitter()
    : pattern_sync_each_ref(false),
      ...
      total_activations(5000000), num_aggs_for_sync(2) {
#ifdef ENABLE_JITTING
  logger = new asmjit::StringLogger;
#endif
}

CodeJitter::~CodeJitter() { cleanup(); }
...
int CodeJitter::hammer_pattern(FuzzingParameterSet &fuzzing_parameters,
                               bool verbose) {
  if (fn == nullptr) {
    Logger::log_error("Skipping hammering pattern as pattern could not be "
  ...
  if (verbose) {
    ...
    auto pattern_rounds =
        fuzzing_parameters.get_hammering_total_num_activations() /
        total_acts_pattern;
    auto acts_per_pattern_round =
        pattern_sync_each_ref
            // sync after each num_acts_per_tREFI: computes how many activations
            // are necessary by taking our pattern's length into account
            ? (total_acts_pattern /
               fuzzing_parameters.get_num_activations_per_t_refi())
            // beginning and end of pattern; for simplicity we only consider the
            // end of the pattern here (=1) as this is the sync that is repeated
            // after each hammering run
            : 1;
    auto num_synced_refs = pattern_rounds * acts_per_pattern_round;
    Logger::log_data(format_string("Number of pattern reps while hammering: %d",
                                   pattern_rounds));
    Logger::log_data(format_string("Number of total synced REFs (est.): %d",
                                   num_synced_refs));
    Logger::log_data(format_string("Avg. number of acts per sync: %d",
                                   total_sync_acts / num_synced_refs));
  }

  return total_sync_acts;
}

void CodeJitter::jit_strict(int num_acts_per_trefi, FLUSHING_STRATEGY flushing,
                            FENCING_STRATEGY fencing,
                            const std::vector<volatile char *> &aggressor_pairs,
                            bool sync_each_ref, int num_aggressors_for_sync,
  ...
  // decides the number of aggressors of the beginning/end to be used for
  // detecting the refresh interval e.g., 10 means use the first 10 aggs in
  // aggressor_pairs (repeatedly, if necessary) to detect the start refresh
  // (i.e., at the beginning) and the last 10 aggs in aggressor_pairs to detect
  // the last refresh (at the end);
  const auto NUM_TIMED_ACCESSES = num_aggressors_for_sync;

  // check whether the NUM_TIMED_ACCESSES value works at all - otherwise just
  // return from this function this is safe as hammer_pattern checks whether
  // there's a valid jitted function
  if (static_cast<size_t>(NUM_TIMED_ACCESSES) > aggressor_pairs.size()) {
    Logger::log_error(format_string(
        "NUM_TIMED_ACCESSES (%d) is larger than #aggressor_pairs (%zu).",
  ...
  if (fn != nullptr) {
    Logger::log_error(
        "Function pointer is not NULL, cannot continue jitting code without "
        "leaking memory. Did you forget to call cleanup() before?");
    exit(1);
  }
  ...
  // ==================================================== The following JIT
  // instructions are based on hammer_sync in blacksmith.cpp, git commit
  // 624a6492.
  ...
  // a map to keep track of aggressors that have been accessed before and need a
  // fence before their next access
  std::unordered_map<uint64_t, bool> accessed_before;

  size_t cnt_total_activations = 0;

  // hammer each aggressor once
  for (int i = NUM_TIMED_ACCESSES;
       i < static_cast<int>(aggressor_pairs.size()) - NUM_TIMED_ACCESSES; i++) {
  ...
  // -----------------------------------------------------------------------
  std::vector<volatile char *> last_aggs(
      aggressor_pairs.end() - NUM_TIMED_ACCESSES, aggressor_pairs.end());
  sync_ref(last_aggs, a);

  a.jmp(for_begin);
  a.bind(for_end);

  // now move our counter for no. of activations in the end of interval sync. to
  // the 1st output register %eax
  a.mov(asmjit::x86::eax, asmjit::x86::edx);
  a.ret(); // this is ESSENTIAL otherwise execution of jitted code creates a
           // segfault
  ...
  Logger::log_error("Cannot do code jitting. Set option ENABLE_JITTING to ON "
                    "in CMakeLists.txt and do a rebuild.");
#endif
}
...
void CodeJitter::sync_ref(const std::vector<volatile char *> &aggressor_pairs,
  ...
  for (auto agg : aggressor_pairs) {
    ...
    // because the number of sync activations varies for different patterns; if
    // we deduct it from the total number of activations, we cannot ensure
    // anymore that we are hammering long enough/as many times as needed to
    // trigger bit flips
